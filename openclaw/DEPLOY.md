# Deploying the Mem0 Fork to Walle

Step-by-step guide for switching walle's openclaw-mem0 plugin from
patched node_modules to the jamebobob/mem0-vigil fork.

**Audience:** A future Opus or Claude Code session deploying to walle
...
## Pre-Deploy Checklist (run on walle)
...
**Verify on walle:** Check that `openclaw.json` now references the
...
### 1. Eve responds on Telegram

- Send a DM to Eve. She should respond normally.
...
- **Don't deploy on a day when walle can't be monitored** for at
  least 2-3 hours afterward. Eve needs to be observed in both DM
  and group contexts.
