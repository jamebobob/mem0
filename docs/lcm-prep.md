...
on morty and read the actual source code.
...
Walle runs v2026.3.13 — **compatible**.
...
3. After compaction, check if Eve still has Safety and Memory Rules sections
...
**Verification needed on walle**: After enabling LCM and triggering compaction, check
...
# 8. Test Q4: Chat with Eve until compaction triggers
#    Send enough messages to exceed 75% of context window
#    After compaction, ask Eve to recall her Safety rules
#    If she can: Q4 = YES
...
1. **Eve stops responding**: Gateway crashed or LCM assembly broke the prompt.
   Action: `sudo systemctl status <gateway-service-name>`, check logs.

2. **Eve responds but has amnesia**: postCompactionSections not working.
   Action: Ask Eve about her Safety rules. If blank: disable LCM.
