...
| Hash | Description |
|------|-------------|
| `278c59a7` | docs(plugin): add deployment guide for switching walle to fork |
...
| File | Purpose |
|------|---------|
| `DEPLOY.md` | Step-by-step walle deployment guide with rollback plan |
...
for the procedure. Requires SSH access to walle.
...
### Walle SSH key setup for Claude Code
No SSH access to walle from this machine. Deploy requires either:
- SSH key exchange, or
- Running the deploy steps manually on walle, or
- A future Claude Code session with walle access
